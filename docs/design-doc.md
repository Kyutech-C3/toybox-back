...
- ブログの投稿プラットフォームとしての機能
  - ブログの作成・編集・保存は別のシステムとして提供する予定であり、このバックエンドにはブログのドメインロジックやデータを持たせない。
  - 移行期間中に限り、公式サイトが必要とするブログ一覧・詳細の読み取りAPIを旧ToyBoxへ透過プロキシする。別システムへの移行完了後に、このプロキシルートと設定を削除する。
...
- デザインや API は 1 から開発
  - ただし、現システムで未実装の公式サイト向け読み取りAPIは、置き換えが完了するまで旧ToyBoxへ限定的にプロキシする。
...
- 旧ToyBox APIからの段階的移行
  - 公式サイト向けの作品一覧API（v1、v2）を読み取り専用でプロキシ
  - ブログ一覧・詳細APIを読み取り専用でプロキシ
  - 現ToyBoxのCookieおよびAuthorizationヘッダーは旧ToyBoxへ送信しない
  - ブログ機能の移行完了後にブログ用プロキシを廃止
...
外部ライブラリを使った実装と、移行期間限定の旧ToyBox向けリバースプロキシを取り扱います。
このプロキシは削除予定であるため依存性注入やドメインエラーなど削除を簡単にするため一部設計から外れています

旧ToyBox向けプロキシは以下を担当します。

- 接続先URLおよび上流Hostの検証
- 上流へのパスとクエリの転送
- 現ToyBoxのCookieおよびAuthorizationヘッダーの除去
- 上流のCORSヘッダーおよびSet-Cookieレスポンスヘッダーの除去
- 上流のレスポンスヘッダー待ちタイムアウト

HTTPステータスとエラーレスポンスの形式はインターフェース層が担当します。
...
- 作品のサムネイルおよび通常アセットは、認証済みの `POST /auth/works/asset` でアップロードします。
- 用途を区別するパラメータはありません
  - サムネイルであろうと同じ検証規則を適用します
- サムネイルのサイズ上限は現状backendでは扱っていません。

### 許可するファイル形式

ファイル名から `filepath.Ext` で最後の拡張子を取得し、先頭の `.` を除いて小文字化したうえで判定します。DB および S3 オブジェクトキーに保存する拡張子もこの小文字です。拡張子がない場合、および次の一覧にない場合は拒否します。3D 形式（gltf、fbx など）は対象外です。

| 種別 | 拡張子 |
| :-- | :-- |
| 画像 | png、jpg、jpeg、bmp、gif、webp |
| 動画 | mp4、mov |
| 音声 | mp3、wav、m4a |
| アーカイブ | zip |
...
ファイル単位のサイズ制限は設けません。multipart として処理できれば受け付けます。  
代わりに、このエンドポイントの POST リクエスト全体に 2GiB の上限を設けます。超過した場合は 413 を返します。他の API にはこの上限を適用しません。

手前にリバースプロキシがある場合は、そちらのリクエストボディ上限も 2GiB 以上にする必要があります。

### S3 の Content-Type

S3 へ保存するオブジェクトの Content-Type には、拡張子に対応する正式な MIME タイプを設定します。Go 標準の `mime.TypeByExtension` は OS の `mime.types` に依存し、Alpine 上では mp4 や zip などが空になり得るため使いません。拡張子と MIME の対応はアプリケーション側で保持します。
...
- 旧ToyBox向けプロキシは公開された読み取りAPIだけを対象とし、現ToyBoxの認証情報を旧システムへ転送しません。また旧システムからのCookieをクライアントへ転送しません。
